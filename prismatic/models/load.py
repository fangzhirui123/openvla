...
def load_vla(
    ...
    if os.path.isfile(model_id_or_path):
        ...
        run_dir = checkpoint_pt.parents[1]#Path("/mnt/petrelfs/fangzhirui/openvla/openvla_7b_full")#checkpoint_pt.parents[1]
    ...
    # Otherwise =>> try looking for a match on `model_id_or_path` on the HF Hub (`VLA_HF_HUB_REPO`)
        ...
        with overwatch.local_zero_first():
            relpath = Path(model_type) / model_id_or_path#Path("/mnt/petrelfs/fangzhirui/openvla/openvla_7b_full")#Path(model_type) / model_id_or_path
    ...
    config_json = '/mnt/petrelfs/fangzhirui/openvla/openvla_7b_full/config.json'
    with open(config_json, "r") as f:
