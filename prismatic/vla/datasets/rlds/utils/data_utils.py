...
# ruff: noqa: B023
def normalize_action_and_proprio(traj: Dict, metadata: Dict, normalization_type: NormalizationType, clamp_value=True):
    """Normalizes the action and proprio fields of a trajectory using the given metadata."""
    keys_to_normalize = {"action": "action", "proprio": "observation/proprio"}
    if 'state' in traj['observation']:
        keys_to_normalize = {"action": "action", "proprio": "observation/proprio", "state": "observation/state"}
    print('clamp_value3', clamp_value)
    if normalization_type == NormalizationType.NORMAL:
        ...
        for key, traj_key in keys_to_normalize.items():
            if normalization_type == NormalizationType.BOUNDS:
                ...
                if key == 'state':
                    t_key = 'proprio'
                low = metadata[t_key]["q01"]
                high = metadata[t_key]["q99"]
            mask = metadata[t_key].get("mask", tf.ones_like(metadata[t_key]["min"], dtype=tf.bool))
            print('clamp_value:', clamp_value, )
            max_val = 1.
            min_val = -1.
            if clamp_value != 1:
                min_val = - clamp_value
                max_val = clamp_value
                pass
            # tf.print('debug:', traj['action'].dtype, traj['observation']['proprio'].dtype,traj['observation']['state'].dtype,)
            if 'state' in traj['observation']:
                traj['observation']['proprio'] = tf.cast(traj['observation']['proprio'], dtype=tf.float32)
                traj['observation']['state'] = tf.cast(traj['observation']['state'], dtype=tf.float32)
            traj = dl.transforms.selective_tree_map(
                traj,
                match=lambda k, _: k == traj_key,
                map_fn=lambda x: tf.where(
                    mask,
                    tf.clip_by_value(2 * (x - low) / (high - low + 1e-8) - 1, min_val, max_val) if clamp_value else 2 * (x - low) / (high - low + 1e-8) - 1 ,
                    x,
                ),
            )

            # Note (Moo Jin): Map unused action dimensions (i.e., dimensions where min == max) to all 0s.
            zeros_mask = metadata[t_key]["min"] == metadata[t_key]["max"]
...
def pprint_data_mixture(dataset_kwargs_list: List[Dict[str, Any]], dataset_weights: List[int]) -> None:
    ...
    for dataset_kwargs, weight in zip(dataset_kwargs_list, dataset_weights):
        ...
        overwatch.info(f"# {dataset_kwargs['name']}: {weight:=>{pad}f} #")
    print("######################################################################################\n")
...
def get_dataset_statistics(
    ...
    # if dataset_name == 'droid_state':
    #     unique_hash= '635dee9cc74d6674def1a0fa1c47766433979baf94d2d5792d4c45921819d546'
    ...
    for traj in tqdm(dataset.iterator(), total=cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else None):
        # tf.reduce_all(traj['action'] 
        if max_action is not None and not tf.math.reduce_all(tf.math.abs(traj["action"][...,:6]) <= max_action).numpy():
            continue
