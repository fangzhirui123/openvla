...
def make_dataset_from_rlds(
    ...
    *,
    ...
    print('clamp_value2', clamp_value)
    def restructure(traj):
        ...
        # if name in ['robo_set', 'aloha_mobile']:
        #     res = joints_to_pose_batch_tf(traj["action"][:, :7])
        #     traj["action"] = tf.concat([res, traj["action"][:, 7:]], axis=-1)
        traj = {
            "observation": new_obs,
            "task": task,
    ...
    if isinstance(dataset_statistics, str):
        ...
        if max_action is not None:
            def func1(x):
                tf.print('stat', max_action, tf.math.abs(x["action"][...,:6]), tf.math.reduce_all(tf.math.abs(x["action"][...,:6]) <= max_action))
...
def apply_trajectory_transforms(
    ...
    *,
    ...
    if max_proprio is not None:
        def func1(x):
            # tf.print('proprio', tf.math.abs(x["proprio"][...,:6]), tf.math.reduce_any(tf.math.reduce_all(tf.math.abs(x["proprio"][...,:6]) != 0,axis=[1,2])))
...
def make_interleaved_dataset(
    ...
    *,
    ...
    # for dataset_kwargs in dataset_kwargs_list:
    #     # print statistics
    #     print(dataset_kwargs["name"], '=======================')
    #     print(all_dataset_statistics[dataset_kwargs["name"]])
    ...
    # print(frame_transform_kwargs)
    # import ipdb;ipdb.set_trace()
    dataset = apply_frame_transforms(dataset, **frame_transform_kwargs, train=train)
    if gen_prompt_transform:
                # Augment all images with the same seed, skipping padding images
        # import ipdb;ipdb.set_trace()
        prompt_builder_fn = gen_prompt_transform_kwargs['prompt_builder_fn']
        action_tokenizer = gen_prompt_transform_kwargs['action_tokenizer']
        num_image_token = gen_prompt_transform_kwargs['num_image_token']
        base_tokenizer = gen_prompt_transform_kwargs['base_tokenizer']
        
        @tf.py_function(Tout=tf.string)
        def obtain_action_prompt(act_inp, lang):
            # print(act_inp)
            # print(gen_prompt_transform)
            # import ipdb;ipdb.set_trace()
            if gen_prompt_transform in [2, 3]:

                # Preprocess the conversations and generate the return dictionary

                from internvl.conversation import get_conv_template
                if gen_prompt_transform == 2:
                ...
                img_place_hodler = ''
                num_image = traj_transform_kwargs['window_size']
                for img_i in range(num_image):
                    img_place_hodler += '<image>\n'
                conversation = [{"from": "human", "value": img_place_hodler + f"What action should the robot take to {lang.numpy()}?"},{"from": "gpt", "value": ','.join(action_tokenizer(act_inp))},]
                ...
                num_image_token_list = [num_image_token * 1 for img_ii in range(num_image)]

                # ret = prompt_builder_fn(template_name, [conversation],
                #                         base_tokenizer, num_image_token_list,
                #                         group_by_length=True, ds_name='')
                # import ipdb;ipdb.set_trace()
                # return {}
            ...
            # import ipdb;ipdb.set_trace()
            conversation = [{"from": "human", "value": f"What action should the robot take to {lang.numpy()}?"},{"from": "gpt", "value": ','.join(action_tokenizer(act_inp))},]
            for turn in conversation:
        ...
        def gen_prompt_for_language_model(frame: dict):
            

            lang = frame["task"]["language_instruction"]
            # action = frame['action'][traj_transform_kwargs['window_size']-1:] # tf.tensor, 
            action = frame['action'] # tf.tensor, 

            # print(type(lang))
            # tf.function(frame['action'])
            # action = action.numpy()

            # Construct Chat-based Prompt =>> Input is default query + language instruction, output are the action tokens
            
            frame['prompt'] = obtain_action_prompt(action, lang)
            
            return frame

        dataset = dataset.frame_map(gen_prompt_for_language_model, threads)
