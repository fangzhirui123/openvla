...
sys.path.append('/mnt/petrelfs/fangzhirui/openvla/')
import re
...
sys.path.append('/mnt/petrelfs/fangzhirui/openvla')
sys.path.append('/mnt/petrelfs/fangzhirui/')
os.environ["AWS_ACCESS_KEY_ID"] = "H9HBOW256ACSZ0G62JGG"
os.environ["AWS_SECRET_ACCESS_KEY"] = "o3fiSkvVaNRsDiLMhqA1unUNYKzWfxnyGTErZLrW"
os.environ["S3_ENDPOINT"] = "http://p-ceph-norm-inside.pjlab.org.cn"
os.environ["S3_USE_HTTPS"] = "0"
...
# #wandb.login(key="4ef3159bf306675daf8dcb93a43f8319de60cd37")
# wandb.login(key="4ef3159bf306675daf8dcb93a43f8319de60cd37")
# wandb.init(
#     # set the wandb project where this run will be logged
#     project="openvla_full_64",
#     # project="openvla",
...
def parse_args():
    ...
    #parser.add_argument('--vla.type', type=str, help="Type of VLA model")
    parser.add_argument('--data_root_dir', type=str, help="Root directory for dataset")
    parser.add_argument('--run_root_dir', type=str, help="Root directory for experiment runs")
    parser.add_argument('--run_id', type=str, help="Run ID for the experiment")
    parser.add_argument('--image_aug', type=bool, default=False, help="Whether to apply image augmentation")
    ...
    parser.add_argument('--is_resume', type=bool, default=False, help="Whether to resume training from a checkpoint")
...
class TrainConfig:
    ...
    vla: VLAConfig = field(
        ...
        #default_factory=VLAConfig.get_choice_class(VLARegistry.Exp_DinoSigLIP_224px_OXE_Magic_Soup_Plus.vla_id)
        #default_factory=VLAConfig.get_choice_class(VLARegistry.Exp_FreezeVIT_DinoSigLIP_224px_Bridge.vla_id)
    )

    # Directory Paths
    data_root_dir: Path = Path(                                     # Path to Open-X dataset directory
        '/mnt/petrelfs/share_data/zhangtianyi1/'
    )
    run_root_dir: Path = Path("runs")                               # Path to directory to store logs & checkpoints

    # Resume Run Parameters
    pretrained_checkpoint: Optional[Path] = '/mnt/petrelfs/fangzhirui/openvla/openvla_7b_full/checkpoints/'                    # Absolute Path to Checkpoint
    ...
    dataset_names: Tuple[str, ...] = ("calvin",)
    n_gpu: int = 8
...
def main(cfg: TrainConfig) -> None:
    ...
    #cfg.vla.type = args.vla.type
    cfg.data_root_dir = Path(args.data_root_dir)
    cfg.run_root_dir = Path(args.run_root_dir)
    cfg.run_id = args.run_id
    cfg.image_aug = args.image_aug
    # cfg.wandb_project = args.wandb_project
    # cfg.wandb_entity = args.wandb_entity
    cfg.save_interval = args.save_interval
    cfg.is_resume = args.is_resume
    # import argparse
    ...
    #print("os.path.isdir(cfg.pretrained_checkpoint)",os.path.isdir(cfg.pretrained_checkpoint))
    #print("os.listdir(cfg.pretrained_checkpoint)",os.listdir(cfg.pretrained_checkpoint))
    if cfg.pretrained_checkpoint is not None and os.path.isdir(cfg.pretrained_checkpoint) and len(os.listdir(cfg.pretrained_checkpoint)) > 0:
        #cfg.pretrained_checkpoint /mnt/petrelfs/fangzhirui/openvla/openvla_7b_full/checkpoints/ os.path.isdir(cfg.pretrained_checkpoint) True len(os.listdir(cfg.pretrained_checkpoint)) 1
        #################################################
        if os.path.isdir(cfg.pretrained_checkpoint) and len(os.listdir(cfg.pretrained_checkpoint)) > 0:
            ...
            #cfg.pretrained_checkpoint /mnt/petrelfs/fangzhirui/openvla/openvla_7b_full/checkpoints/step-295000-epoch-40-loss=0.2200.pt
    ...
    print("cfg.vla.freeze_vision_backbone",cfg.vla.freeze_vision_backbone,"cfg.vla.freeze_llm_backbone",cfg.vla.freeze_llm_backbone)
    #cfg.vla.freeze_vision_backbone False cfg.vla.freeze_llm_backbone False
...
# }
    ...
    #print("cfg.per_device_batch_size",cfg.per_device_batch_size)
    ...
    # dataset_names = vla_dataset.pop('dataset_name')
    # img_orig = vla_dataset.pop('img_orig')   #<PIL.Image.Image image mode=RGB size=224x224 at 0x7F130826D420>}
    ...
    class MyTrainer(Trainer):
        ...
        def compute_loss(self, model, inputs, return_outputs=False):
            #print("inputs",inputs)
            # print("inputs['pixel_values']['dino']",inputs['pixel_values']['dino'].shape)
            # print("inputs['pixel_values']['siglip']",inputs['pixel_values']['siglip'].shape)
            # inputs['pixel_values']['dino'] torch.Size([3, 224, 224])
            # inputs['pixel_values']['siglip'] torch.Size([3, 224, 224])
            ...
            # 如果 input_ids 只有一个维度，则扩展维度
            # if inputs['input_ids'].ndimension() == 1:
            #     inputs['input_ids'] = inputs['input_ids'].unsqueeze(0)  # 形状变为 [batch_size, 1]
            
            
            if 'DEBUG' in os.environ:
                print("model.vision_model",model.vision_model)
            ...
            #print("model.vision_backbone.dino_featurizer.patch_embed.num_patches",model.vision_backbone.dino_featurizer.patch_embed.num_patches)
            #print("model.vision_backbone.siglip_featurizer.patch_embed.num_patches",model.vision_backbone.siglip_featurizer.patch_embed.num_patches)
            #   256 256
            action_preds = output.logits[:, model.vision_backbone.dino_featurizer.patch_embed.num_patches : -1].argmax(dim=2)
            ...
            if dist.get_rank() == 0:
                ...
                try:
                    ...
                    if dist.get_rank() == 0:
                        datasets = set(cfg.dataset_names)
                        # print("datasets",datasets)
                        if len(datasets) > 1:
                            for ds in datasets:
                                ds_mask = torch.tensor([elem == ds for elem in cfg.dataset_names])
                                # print("mask",mask.shape)
                                # print("ds_mask",ds_mask.shape)
                                # print("correct_preds",correct_preds.shape)
                    ...
                    epoch = (metrics.global_step + 1) // (len(vla_dataset) // (cfg.n_gpu*training_args1.per_device_train_batch_size))
    ...
    training_args1 = TrainingArguments(
        ...
        deepspeed="/mnt/petrelfs/fangzhirui/openvla/vla-scripts/zero_stage3_config_vla.json"  # Specify your DeepSpeed config file
    )
    ...
    # Start training
    # import ipdb;ipdb.set_trace()
#     print("vlm.vision_backbone.dino_featurizer.blocks[0].norm1.weight",vlm.vision_backbone.dino_featurizer.blocks[0].norm1.weight)
#     vlm.vision_backbone.dino_featurizer.blocks[0].norm1.weight Parameter containing:
# tensor([ 5.0345e-04,  1.3406e+00, -2.5369e-04,  ...,  3.2968e-01,
#          1.1566e-02, -6.6864e-04], requires_grad=True)
    trainer.train()
...
if __name__ == "__main__":
    ...
    # print("SLURM_STEP_NODELIST",SLURM_STEP_NODELIST)
    #import subprocess
