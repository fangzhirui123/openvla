...
class CustomModel1(CalvinBaseModel):
    def __init__(self, model, cfg):
        ...
        if cfg.single_gpu_running:
            ...
            os.environ["MASTER_PORT"] = '12345'+1000*os.environ["LOCAL_RANK"]
...
class CustomModel_full(CalvinBaseModel):
    def __init__(self, model, cfg):
        ...
        training_args1 = TrainingArguments(
            output_dir='./results',  # 保存输出结果的目录
            per_device_eval_batch_size=1,  # 每次评估的批大小
            no_cuda=False,  # 是否使用 GPU
            logging_dir='./logs',  # 日志存储目录
            prediction_loss_only=True,  # 仅返回预测结果
            overwrite_output_dir=True  # 如果存在，覆盖输出目录
        )
        ...
        self.vla = trainer._wrap_model(self.vla, training=False, dataloader=None)
    ...
    def step(self, obs, goal):
        ...
        # self.rollout_step_counter += 1
        #print("action1",action)
#         action1 [[ 0.06731646 -0.07050718 -0.23308485 -0.1708287   0.20175324 -0.05357677
#    1.        ]]
# action1 [[-0.26524762  0.05606681  0.08264848  0.15889651 -0.36661279 -0.01051625
#    1.        ]]
# action1 [[ 0.3074507  -0.05270937 -0.13879896 -0.24527228  0.04328366 -0.1061627
#   -1.        ]]
# action1 [[ 0.13597546 -0.01310207  0.02336207 -0.07277935  0.04059555  0.1282859
#   -1.        ]]
# action1 [[ 0.21797131 -0.12346136 -0.12697156 -0.19270157 -0.00823486 -0.10318517
#   -1.        ]]
...
def close_loop_eval_calvin(
    ...
    def make_env(dataset_path):
        ...
        #os.environ["EGL_VISIBLE_DEVICES"] = str(args.rank)
