...
def evaluate_policy(model, env, epoch, eval_log_dir=None, debug=False, create_plan_tsne=False, rank=0, each_length=1000):
    ...
    # import ipdb;ipdb.set_trace()
    eval_sequences = get_sequences(NUM_SEQUENCES, num_workers=8)
    eval_sequences = eval_sequences[rank*each_length:(rank+1)*each_length]
    # eval_sequences 1000
    # each_length 100 eval_sequences1 100
    #({'led': 0, 'lightbulb': 1, 'slider': 'left', 'drawer': 'closed', 'red_block': 'slider_left', 'blue_block': 'table', 'pink_block': 'slider_right', 'grasped': 0}, ('rotate_blue_block_left', 'turn_off_lightbulb', 'move_slider_right', 'open_drawer', 'lift_red_block_slider'))
    ...
    with open(eval_log_dir / 'results_orig.json', 'w') as file:
        json.dump({'results': results, 'sequences': orig_eval_sequences}, file)        
    if create_plan_tsne:
