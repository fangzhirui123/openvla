...
def evaluate_sequence(env, model, task_checker, initial_state, eval_sequence, val_annotations, plans, debug):
    ...
    if debug:
        ...
        #print("Subtask: ", end="")
    # [val_annotations[item][0] for item in eval_sequence] 
...
def evaluate_policy_fullang(model, env, epoch, eval_log_dir=None, debug=False, create_plan_tsne=False, rank=0, each_length=1000):
    """
    Run this function to evaluate a model on the CALVIN challenge.
    ...
    iii = 0
    for initial_state, eval_sequence in eval_sequences:
        #print("eval_sequences",eval_sequences)
        # eval_sequences 1/5 : 0.0% | 2/5 : 0.0% | 3/5 : 0.0% | 4/5 : 0.0% | 5/5 : 0.0% ||:   3%|▎         | 3/100 [52:00<27:58:17, 1038.12s/it]
        #print("initial_state",initial_state,"eval_sequence",eval_sequence,"eval_sequences",eval_sequences)
        ...
        # eval_sequence ('rotate_blue_block_right', 'move_slider_right', 'lift_red_block_slider', 'place_in_slider', 'turn_off_lightbulb')
        result = evaluate_sequence_fullang(env, model, task_oracle, initial_state, eval_sequence, val_annotations, plans, debug)
        #print("result",result)
        results.append(result)
        if not debug:
            eval_sequences.set_description(
...
def evaluate_sequence_fullang(env, model, task_checker, initial_state, eval_sequence, val_annotations, plans, debug):
    """
    Evaluates a sequence of language instructions.
    """
    robot_obs, scene_obs = get_env_state_for_initial_condition(initial_state)
    ...
    for subtask in eval_sequence:
        ...
        # subtask rotate_blue_block_right
        success = rollout_fullang(env, model, task_checker, subtask, val_annotations, plans, debug, eval_sequence)
        if success:
            success_counter += 1
        else:
            return success_counter
...
def rollout_fullang(env, model, task_oracle, subtask, val_annotations, plans, debug, eval_sequence):
    """
    Run the actual rollout on one subtask (which is one natural language instruction).
    """
    if debug:
