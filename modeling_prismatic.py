...
class PrismaticForConditionalGeneration(PrismaticPreTrainedModel):
    ...
    def forward(
        ...
        # print("past_key_values1",past_key_values)
        # #   past_key_values1 None
        # print("attention_mask1",attention_mask.shape)
        # #   torch.Size([16, 36])
        # print("labels1",labels.shape)
        # #   labels1 torch.Size([16, 36])
        
        # pixel_values1 torch.Size([16, 6, 224, 224])
        # input_ids1 torch.Size([16, 36])
...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def predict_action(
        self, input_ids: Optional[torch.LongTensor] = None, pixel_values: Optional[torch.LongTensor] = None, unnorm_key: Optional[str] = None, non_autoregressive: bool = False, **kwargs: str
    ) -> np.ndarray:
        ...
        # Run VLA inference
        if not non_autoregressive:
            #print("input_ids22222222222222",input_ids)
            generated_ids = self.generate(input_ids, max_new_tokens=self.get_action_dim(unnorm_key), **kwargs)

        if non_autoregressive:
            input_ids = torch.cat(
                (input_ids, torch.unsqueeze(torch.Tensor([32000]*7).long(),dim=0).to(input_ids.device)), dim=1
            ) 
            pixel_values = pixel_values.to(torch.bfloat16).to(input_ids.device)
            #pixel_values['siglip'] = pixel_values['siglip'].to(torch.bfloat16)
            #print("input_ids111111111111",input_ids)
            #print("input_ids.shape",input_ids.shape)
            results = self(input_ids=input_ids,pixel_values=pixel_values)
            # print("results.logits.argmax(-1)",results.logits.argmax(-1))
            # index_of_2 = torch.nonzero(results.logits.argmax(-1) == 2).squeeze(1).item()
            # start_index = max(0, index_of_2 - 7)
            # generated_ids = results.logits.argmax(-1)[start_index:min(index_of_2,7)]
